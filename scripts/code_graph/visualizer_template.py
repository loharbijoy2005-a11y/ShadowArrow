        ...
        <div id="architecture-container" class="absolute inset-0 z-40 bg-[#0b0f19] hidden flex-col items-center justify-start sm:justify-center p-4 sm:p-8 overflow-y-auto pt-16 sm:pt-8">
            <div class="max-w-5xl w-full flex flex-col items-center text-center pb-10">
                ...
                <div class="mb-8 w-full max-w-sm">
                    <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Simulate Attack Scenario</label>
                    <select id="attack-simulator" class="w-full bg-slate-900 border border-slate-700 rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer">
                        <option value="none">🟢 Normal Secure Flow</option>
    ...
    <script type="text/javascript">
        ...
        function resetNodes() {
            ...
            allNodes.forEach(id => {
                ...
                if(el) {
                    el.style.opacity = '1';
                    el.classList.remove('bg-red-900/50', 'border-red-500/50');
                    // restore original colors
                    if(id === 'node-firewall') { el.classList.add('bg-slate-800', 'border-rose-900/50'); el.querySelector('.status-text').innerText = 'Rate Limit & DDoS Check'; el.querySelector('.status-text').classList.remove('text-red-400', 'font-bold'); el.querySelector('.status-text').classList.add('text-slate-400'); }
                    if(id === 'node-mongo') { el.classList.add('bg-emerald-900/50', 'border-emerald-500/50'); el.querySelector('.status-text').innerText = 'Verifies Real Price'; el.querySelector('.status-text').classList.remove('text-red-400', 'font-bold'); el.querySelector('.status-text').classList.add('text-slate-400'); }
                }
            });
        ...
        function applyHack(scenario) {
            ...
            if (scenario === 'ddos' || scenario === 'path_traversal') {
                cutoffIndex = 1; // Blocked at Firewall (index 1)
                const fw = document.getElementById('node-firewall');
                fw.classList.remove('bg-slate-800', 'border-rose-900/50');
                fw.classList.add('bg-red-900/50', 'border-red-500/50');
                const text = fw.querySelector('.status-text');
                text.innerText = 'ATTACK BLOCKED ❌';
                text.classList.remove('text-slate-400');
                text.classList.add('text-red-400', 'font-bold');
            } else if (scenario === 'price_hack') {
                cutoffIndex = 3; // Blocked at Mongo (index 3)
                const mongo = document.getElementById('node-mongo');
                mongo.classList.remove('bg-emerald-900/50', 'border-emerald-500/50');
                mongo.classList.add('bg-red-900/50', 'border-red-500/50');
                const text = mongo.querySelector('.status-text');
                text.innerText = 'FAKE PRICE REJECTED ❌';
                text.classList.remove('text-slate-400');
                text.classList.add('text-red-400', 'font-bold');
            }

            // Gray out nodes and arrows after cutoff
