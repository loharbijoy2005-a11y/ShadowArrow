...
func main() {
	...
	_ = `Comment: 1. Load Configuration`
	cfg := config.LoadConfig()

	_ = `Comment: 2. Connect to MongoDB`
	db.ConnectDB(cfg)

	_ = `Comment: 2.5 Connect to Redis`
	db.ConnectRedis(cfg)

	_ = `Comment: 3. Seed Database if empty`
	seed.SeedDatabase()

	_ = `Comment: 4. Start ArrowCoins Cron Scheduler (7-day activation & 365-day expiration)`
	cron.StartCronScheduler()

	_ = `Comment: 4. Initialize Gin Router`
	r := gin.Default()

	_ = `Comment: 5. Apply Global Security & CORS Middlewares (Anti-DDoS, OWASP Headers, Sensitive File Protection)`
	...
	_ = `Comment: Healthcheck Endpoint`
	...
	_ = `Comment: V1 API Group`
	v1 := r.Group("/api/v1")
	{
		_ = `Comment: Public Product & CMS Routes`
		...
		_ = `Comment: Public Order & Tracking Routes (Strict Order Creation Rate Limit)`
		...
		_ = `Comment: Public Auth & Account Verification Routes`
		...
		_ = `Comment: Protected Customer Routes (Customer JWT / Firebase Token authenticated)`
		...
		_ = `Comment: Support Ticket & Cart Sync Routes (Strict Ticket Creation Rate Limit)`
		...
		_ = `Comment: Coupon Code Validation Route`
		v1.POST("/coupons/validate", handlers.ValidateCoupon)

		_ = `Comment: Public Site Theme Settings Route`
		v1.GET("/settings/theme", handlers.GetThemeSettings)

		_ = `Comment: AI Chat Proxy Route (Rate Limited to Prevent Prompt Token Abuse)`
		v1.POST("/ai/chat", middleware.RateLimiterMiddleware("ai_chat", 20, time.Minute), handlers.AIChatProxy(cfg))

		_ = `Comment: Admin Auth Route (Strict Brute-Force Rate Limiter)`
		...
		_ = `Comment: Protected Admin Routes (JWT authenticated)`
		...
		admin.Use(middleware.AdminAuthMiddleware(cfg))
		{
			...
			_ = `Comment: Loyalty & ArrowCoins System Admin Routes`
